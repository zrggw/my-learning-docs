...
## 阶段 12：文件操作（标准库 `<stdio.h>`）

> 重点：**`fopen` 的模式（r/w/a + 组合 + 二进制）**及各模式行为，以及**关键函数在不同情况下的返回值**。这是工作量最大、坑也最多的一块。

- [ ] **`fopen(path, mode)`**：打开文件，返回 `FILE*`（文件指针）或 `NULL`。
  - `mode` 是字符串，决定"读/写/追加、文本或二进制、是否清空、位置"
  - 返回值：**成功** → 非空 `FILE*`；**失败**（文件不存在、权限不足、路径错误等）→ `NULL`。**判空是必做检查**：
    ```c
...
| 模式 | 允许操作 | 文件不存在 | 文件已存在 | 打开时文件位置 | 是否清空内容 |
|---|---|---|---|---|---|
| `"r"` | 读 | 失败（`NULL`） | 保留内容，只读 | 开头 | 否 |
| `"w"` | 写 | **创建** | **清空/截断** | 开头 | **是** |
| `"wx"` | 写（独占，C11） | **创建** | **失败（`NULL`）** | 开头 | 否 |
| `"a"` | 追加写 | **创建** | 保留内容 | **末尾** | 否 |
| `"r+"` | 读+写 | 失败 | 保留内容，可读可写 | 开头 | 否 |
| `"w+"` | 读+写 | **创建** | **清空/截断** | 开头 | **是** |
| `"w+x"` | 读+写（独占，C11） | **创建** | **失败（`NULL`）** | 开头 | 否 |
| `"a+"` | 读+写（追加） | **创建** | 保留内容 | 读在开头 / 写在末尾 | 否 |

- 关键结论：
  - **`"r"` 系列文件必须已存在**，否则 `NULL`；**`"w"` 系列会清空**已存在文件、不存在则创建；**`"a"` 系列不清空**、只在末尾追加
  - **`+` 表示"可读可写"**，是读写的扩展；不带 `+` 只能单向
  - **`x`（C11 独占）**：**文件已存在直接失败**，绝不覆盖——写模式里唯一"防覆盖"的选项
  - **写模式（`w`/`a`）会自动创建不存在的文件**；读模式（`r`）不会
  - **`a`/`a+` 无论怎么写都写到末尾**（即使你曾用 `fseek` 移动过位置，追加模式写也强制落尾）

- [ ] **二进制模式 `b`**：在模式后加 `b`，如 `"rb"` / `"wb"` / `"ab"` / `"r+b"` / `"w+b"` / `"a+b"`。
  - 文本模式（无 `b`）：Windows 下 `\n` 与 `\r\n`、文件尾 `0x1A` 会**被转换**；`fread`/`fwrite` 时数据可能"变了样"
  - **二进制模式**：不做任何转换，**直接搬字节**——读写**非文本数据**（字节、结构体、图像、音频）必须用 `b`
  - 在 Linux/macOS 文本和二进制无差别；**Windows 上混用带 `b` 与不带 `b` 是常见坑**
  - 组合记忆：**模式 = 读写加减号 + 可选的 `b`**。`"r+b"`（可读可写二进制）、`"w+b"`（清空可读写二进制）、`"a+b"`（追加可读二进制）
  - 上述每种写模式还可再加 `x`（C11 独占），如 `"wbx"` / `"w+bx"`（二进制 + 独占）。**记忆：只有 `x` 能防止覆盖，其余写模式对已存在文件默认"优先清空"。**

- [ ] **C11 独占创建模式 `x`（重点，防覆盖）**：在写相关模式后加 `x`，如 **`"wx"`** / **`"w+x"`** / **`"wbx"`** / **`"w+bx"`**。
  - **`x` 的含义 = "独占创建（exclusive）"**：只有**文件不存在**时才打开；**若文件已存在则直接失败**（返回 `NULL`），**绝不会清空/覆盖已有文件**
  - 与普通 `"w"` 的区别：`"w"` 会**先清空**已存在文件（可能覆盖重要数据）；`"wx"` **拒绝覆盖**，是安全创建新文件的 C11 标准写法
  - 场景：防止误覆盖已有数据（日志轮转、生成新文件、原子写）、避免两个进程同时创建同一文件（`"wx"` 可作简单的"锁/占用"标记）
  - **返回值**：成功 → 非空 `FILE*`；**文件已存在** → `NULL`（且 `errno` 通常设为 `EEXIST`，可用 `perror` 看出"File exists"）
  - 注意：`x` **只和"写"模式组合**（`w`/`w+`/`wbx`/`w+bx`）；读模式 `r`、追加 `a` 都没有 `x`；**C11 才引入**，老编译器/老代码可能报错，需 `C11` 以上
  ```c
  FILE *fp = fopen("out.txt", "wx");   // 独占创建：已存在则失败
  if (!fp) { perror("create failed"); return -1; }   // 文件已存在时输出 "create failed: File exists"
  ```
...
- [ ] **`fclose(fp)`**：关闭文件，**成功返回 0，失败返回 `EOF`（-1）**。忘关会造成资源泄漏/缓冲未刷新。

### 12.2 关键函数的返回值（必考）

- [ ] **`fread(ptr, size, nmemb, fp)`**：从 `fp` 读 `nmemb` 个、每个 `size` 字节，存到 `ptr`。
  - **返回值 = 实际读到的"元素个数"**（不是字节数！）。正常应等于 `nmemb`；小于则可能是 `EOF` 或出错
  - 判断读多少：`n = fread(...)`，若 `n < nmemb` 用 `feof(fp)` / `ferror(fp)` 区分"读完"还是"出错"
  - 要读**一个完整结构体**（二进制）：
    ```c
    size_t n = fread(&st, sizeof(st), 1, fp);   // 读 1 个 st
    if (n != 1) { /* 没读到完整结构体：EOF 或出错 */ }
    ```
- [ ] **`fwrite(ptr, size, nmemb, fp)`**：写 `nmemb` 个、每个 `size` 字节。
  - **返回值 = 成功写入的"元素个数"**。小于 `nmemb` → 写失败（磁盘满、权限等）
  - 常配合判断：`if (fwrite(...) != nmemb) { /* 写失败 */ }`
- [ ] **`fscanf(fp, fmt, ...)`**：从文件格式化读。
  - **返回值 = 成功匹配并赋值的参数个数**（`>0` 读到了；`0` 没匹配上；`EOF` 遇到文件尾或出错）
  - 判文件尾：`while (fscanf(fp, "%d", &x) != EOF) { ... }`
- [ ] **`fprintf(fp, fmt, ...)`**：格式化写到文件；**返回值 = 写入的字符数**，为负表示出错。
- [ ] **`feof(fp)` / `ferror(fp)`**：
  - `feof(fp)`：文件尾标志 → 返回**非 0** 表示已到文件尾
  - `ferror(fp)`：出错标志 → 返回**非 0** 表示发生了错误
  - 二者**互补**：`fread`/`fscanf` 返回值偏小后，用它们判断"是正常读完还是出错"。注意 **`feof` 只有读越过文件尾才置位**，别在循环条件里直接用它判尾（可能多读一次）
- [ ] **`fseek(fp, offset, origin)`**：移动读写位置（随机访问）。
  - `origin`：`SEEK_SET`（文件头）、`SEEK_CUR`（当前位置）、`SEEK_END`（文件尾）
  - **成功返回 0，失败返回非 0**
  - 常用于二进制文件随机读写，或拿文件大小：`fseek(fp, 0, SEEK_END); long sz = ftell(fp);`
- [ ] **`ftell(fp)`**：返回当前位置（`long`），失败返回 `-1L`；配合 `fseek` 可测文件长度。
- [ ] **`perror(s)` / `strerror(errno)`**：错误时打印人类可读信息：
  ```c
  FILE *fp = fopen("x", "r");
  if (!fp) { perror("x"); return -1; }   // 输出：x: No such file or directory
  ```

### 12.3 读写流程与切换模式（先读后写 / 读写混合）

- [ ] **从"读"切到"写"（或反之）**：必须用 `fseek` / `fflush` 重新定位，否则行为未定义：
  ```c
  fseek(fp, 0, SEEK_SET);   // 改写前先定位（读->写 或 写->读 之间必须定位或刷新）
  ```
- [ ] **`fflush(fp)`**：把缓冲数据刷到文件；写日志/中途意外崩溃时用它防盗（成功 0，失败 EOF）。
- [ ] **一个完整例子（复制文件）**：
  ```c
  FILE *in = fopen("a.bin", "rb"), *out = fopen("b.bin", "wb");
...
> 与 C++ 对照：C++ 的 `ifstream`/`ofstream`/`fstream` 封装了这些，构造函数里开、析构自动关（RAII）；C 只有 `fopen`/`fclose` 手动。**模式/返回值语义与 C 一致**，但 C 必须手动判断 `NULL` 与 `fread/fwrite` 返回值。读二进制时二者都会忽略文本转换，C 靠 `"b"`，C++ 靠 `std::ios::binary`。
...
## 阶段 14：基本数据类型字节大小（跨平台对照）

> 重点：**C 标准没有规定 `int` 一定是几字节**，只规定"至少多大 + 相对顺序"（`char <= short <= int <= long <= long long`）。具体字节数由**数据模型（data model）**决定，Windows 与 Linux 在 64 位下**故意不同**——这是跨平台移植最容易踩的坑。

### 14.1 主流平台字节大小对照表

| 数据类型 | Windows 32 位 | Windows 64 位 | Linux 32 位 | Linux 64 位 |
|---|---|---|---|---|
| `char` | 1 | 1 | 1 | 1 |
| `short` | 2 | 2 | 2 | 2 |
| `int` | 4 | 4 | 4 | 4 |
...
| `float` | 4 | 4 | 4 | 4 |
| `double` | 8 | 8 | 8 | 8 |
| `long double` | 8 | 8 | 12（x86） | 16（x86-64） |
| 指针 `void*` | 4 | **8** | 4 | **8** |
| `size_t` | 4 | 8 | 4 | 8 |

> 单位：字节（byte）。`long double` 因架构而异（见下方说明），其余为 Windows(MSVC) / Linux(gcc/clang + glibc) 的典型值。

### 14.2 关键结论（务必记住）

- [ ] **`int` 在四个平台上都是 4 字节**——这是最稳的假设，跨平台代码可放心用。
- [ ] **`long` 是最大的不一致点**：
  - **Windows 64 位**用 **LLP64** 模型：`long` 仍是 **4 字节**，只有 `long long` 和指针是 8 字节
  - **Linux 64 位**用 **LP64** 模型：`long` 是 **8 字节**
  - 后果：`sizeof(long)` 在 Win64 是 4、在 Linux64 是 8；用 `long` 存指针值或大整数会**在 Windows 上截断**
- [ ] **指针 / `size_t` 随位宽变化**：32 位平台 4 字节，64 位平台 8 字节。**指针大小 = 平台位宽**，这也是"32 位程序不能直接用 64 位库"的原因。
- [ ] **`float` 恒为 4、`double` 恒为 8**（IEEE 754 单/双精度），四平台一致，可放心。
- [ ] **`long double` 最不可移植**：
  - Windows(MSVC)：`long double` **等同 `double`**，8 字节
  - Linux x86 32 位：80 位扩展精度，`sizeof` 通常为 **12**
  - Linux x86-64：80 位扩展精度，按 16 字节对齐存储，`sizeof` 为 **16**
  - ARM 上又可能不同 → **跨平台别用 `long double`**
- [ ] **`char` 恒为 1（标准强制）**，但 `char` 是否有符号（`signed`/`unsigned`）是**实现定义**的，跨平台不要假设。

> 与 C++ 对照：这些类型大小规则 **C 与 C++ 完全一致**（同属上述数据模型）。C++ 额外多了 `long long` 之外的定宽类型（`<cstdint>`），C 也有 `<stdint.h>`（见下）。

### 14.3 定宽类型：跨平台该用谁

- [ ] **别依赖 `int`/`long` 的具体大小**，需要精确位宽时用 `<stdint.h>` 的**定宽整数**：
  | 你想要的 | 用这个 | 保证 |
  |---|---|---|
  | 精确 8 位 | `int8_t` / `uint8_t` | 一定是 1 字节 |
  | 精确 16 位 | `int16_t` / `uint16_t` | 一定是 2 字节 |
  | 精确 32 位 | `int32_t` / `uint32_t` | 一定是 4 字节 |
  | 精确 64 位 | `int64_t` / `uint64_t` | 一定是 8 字节（若平台支持） |
  | 至少 N 位 | `int_leastN_t` | 至少 N 位 |
  | 最快 N 位 | `int_fastN_t` | 至少 N 位且最快 |
  | 能放下指针的整数 | `intptr_t` / `uintptr_t` | 与指针同宽 |
  | 数组下标 / 大小 | `size_t`（`<stddef.h>`） | 无符号、能表示最大对象大小 |
  | 指针差值 | `ptrdiff_t` | 有符号 |
- [ ] **一句口诀**：**要"够用"用 `int`，要"精确"用 `intN_t`，要"大小/下标"用 `size_t`，要"装指针"用 `intptr_t`。**
- [ ] **打印定宽类型的格式符**（用 `<inttypes.h>` 的宏，避免手写 `%lld`/`%ld` 踩平台差异）：
  ```c
  #include <inttypes.h>
  ...
  printf("%" PRIu64 "\n", b);
  ```
  - 直接写 `%ld` 在 Win64/Linux64 会因 `long` 宽度不同而错；用 `PRId64` 等宏才可移植
- [ ] **验证大小（写代码时自测）**：
  ```c
  #include <stdio.h>
  int main(void) {
      printf("int=%zu long=%zu ptr=%zu size_t=%zu\n",
             sizeof(int), sizeof(long), sizeof(void*), sizeof(size_t));
      return 0;
  }
  ```

> 与 C++ 对照：C++ 用 `<cstdint>`（`std::int32_t` 等）与 `<cinttypes>`，机制与 C 相同；C++ 还可用 `static_assert(sizeof(int)==4)` 做编译期校验，C11 起用 `_Static_assert`。**跨平台工程里两者都推荐用定宽类型 + 编译期断言。**

...
- [ ] **非左值直接编译失败**（反证）：`HASH_ADD_INT((T *)0, id, s)` → `error: lvalue required as left operand of assignment`。
  - **什么是左值**：能放在赋值号左边、拥有存储位置（可取地址）的表达式。`users`（变量名）、`*phead`（解引用）、`db->users`（结构体成员）都是左值；`(T *)0`（强制转换结果）、`get_head()`（函数返回值）、`p + 1`（算术表达式）都不是。
  - 宏展开是**文本替换**：实参写 `*phead`，展开后是 `(*phead) = (add);`（合法）；实参写 `(T *)0`，展开后是 `((T *)0) = (add);`（编译失败）。
- [ ] **四种写法的实测结论**：
